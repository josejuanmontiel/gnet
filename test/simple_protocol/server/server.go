...
import (
	...
	protocol "github.com/gnet-io/gnet-examples/simple_protocol/protocol"
)
...
func (s *simpleServer) OnOpen(c gnet.GnetConn) (out []byte, action gnet.Action) {
...
func (s *simpleServer) OnClose(c gnet.GnetConn, err error) (action gnet.Action) {
	if err != nil {
...
func (s *simpleServer) OnTraffic(c gnet.GnetConn) (action gnet.Action) {
	codec := c.Context().(*protocol.SimpleCodec)
	var packets [][]byte
	for {
		data, err := codec.Decode(c.(gnet.Conn))
